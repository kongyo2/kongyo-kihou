...
| 層 | 拘束 |
| --- | --- |
| 打鍵ごと | 七つの必須項の欠落、相対表現の期日、語で書かれた確率、選言、連言、禁止語彙、自判、態度だけの処置を、その場で却下する |
...
3. **週一で末尾記号を置換する。** 期日が来た行には CodeLens で `○` `×` `－` が並ぶ。理由は書かない
4. **月一で集計を見る。** ステータスバーの `×n －n B=0.xxx` をクリックする

### 直接書く

`.kongyo` ファイル、または Markdown 中の ```` ```kongyo ```` フェンスの中で、`P1` と打って補完を受ける。七項の骨格が入り、欠けている項は行末に `⟨欠落 D・R⟩` と出る。却下が消えたら CodeLens の「確定して台帳へ」を押す（`Ctrl+Alt+Enter`）。
